...
**The construction is the deliverable, not that drawing.** Two marks are shipped so the claim can be checked: `signet-{l,m,s}.svg` for the Dev Companion prototype, `tryout-signet-{l,m,s}.svg` for TYPO3 Tryout. Frame, marker, stroke, rounding and gap are identical; only the interior differs, and it carries exactly one idea — a session ending in an answer, or the triangle you press to start an instance. Tryout's triangle is filled *and* stroked at the mark's own weight, the way the corner marker is, so its three points come out at 3.5 through the round join rather than through a second construction that would have to be kept in step. A product adopting this system draws its own interior and touches nothing else.
