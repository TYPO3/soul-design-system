...
/* A URL opens as a URL. A page that loads an ES module cannot have its
   picture taken over `file://`. The browser blocks module scripts there, so
   every element stays unupgraded and the picture is of a page nobody will ever
   see. The rendered site has its own container for exactly this reason. A card
...
/* `withBrowser` rather than a launch of its own. It closes the browser when
   this throws *and* on a signal. A script that opens one by itself is a
   script that can leave one behind. */
await withBrowser(async (browser) => {
  for (const mode of modes) {
    ...
    try {
      ...
      /* The mode goes onto `<html>` rather than through the emulated
         preference. That is what the theme switch does, and the picture has
         to show the state a reader can put the page in. */
      ...
      /* A full-height picture is always of the top of the page, whatever the
         scroll position. So a request for a scroll is a request for the screen
         at that point, which is the only place a pinned thing exists. */
