...
/* Which design system at claude.ai/design a sync uploads into, and how to set it.
   ...
   The app addresses an uploaded system by a project id. That id is what makes a
   second sync an update rather than a second system. The shell cannot look it
   up, as the API authorises a claude.ai login. So this owns everything else:
   which source answered, that the id looks like one, and where it lives.
...
const SOURCES = [
  ...
  ['.design-sync/config.json', 'committed — a fork can keep one here', readId(COMMITTED)],
] as const;

/* One sentence to paste into Claude Code, which does the whole of it. The id
   lives behind a claude.ai login this task cannot reach. The agent that can
   also has a shell, so nobody has to copy a uuid by hand. */
...
if (args.includes('--forget')) {
  ...
  report.fact('forgotten', dropped ? String(dropped) : '(there was no id)');
  report.fact('cleared', 'the cached anchor and the upload plan');
  if (process.env['SDS_DESIGN_PROJECT']) {
    report.note('SDS_DESIGN_PROJECT still stands in this shell and outranks the file — unset it');
  }
...
if (given) {
  ...
  /* A clone that already names a design system keeps it. Whoever asks for this
     ...
     points the next sync at somebody else's system, over an id this clone got
  ...
  if (current && current !== given && !force) {
    ...
    report.summary('nothing written', ['a different project id already stands here'], { shown: true });
    process.exit(1);
  }
  const kept = existsSync(LOCAL) ? JSON.parse(readFileSync(LOCAL, 'utf8')) : {};
  ...
  /* An export outranks the file just written. That is a sync that lands in a
     project the reader did not name and no other output explains. */
  ...
  report.summary('this clone now names the project');
  process.exit(0);
}
...
if (!winner) {
  report.note('no project id anywhere — every sync creates a new design system instead of an update to one');
  howToGetOne();
