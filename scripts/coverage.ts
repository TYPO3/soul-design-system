#!/usr/bin/env node
/* Where a component has to be visible, and what an implementation can invent.

   A component that exists only in `src/` is one the next person writes again.
   Three surfaces have to show it. The story, which the card comes from. The
   class layer, where a name nothing draws rots unchecked. And the Guides
...
/* What the rule does not hold for yet.
   ...
   an absent one, so it cannot quietly outlive the work it stands for. That is
   what turns it into a work list rather than an exemption. */
const PENDING = {
  stories: [] as string[],
  classes: [] as string[],
  /* Not in the Guides render yet. Each needs a node the renderer emits or a
     ...
     element the theme addresses brings everything it draws with it, so the
     figure covers a figure's viewer. */
  guides: [] as string[],
...
/* What a document has no node for — not a work list, and never becomes one.
   ...
   Held to `PENDING`'s discipline from the other side. One of these in the
   render means this classification is wrong, and the check says so. */
const ELSEWHERE = [
  /* A document's headings carry no kind-line: what a section is, its heading
     says. The eyebrow is product-page furniture. */
  'sds-eyebrow',
  /* A reader reads a page and fills nothing in. The bar's search draws
     `.sds-field`, which is the class layer's name and not this element. */
  'sds-field',
  'sds-select',
  'sds-textarea',
  'sds-field-group',
  ...
  'sds-file',
  'sds-form-errors',
  /* How far a running job has got. A page renders before the server sends it,
     so nothing in a document still runs. A bar over finished work is a
     distance claimed about a job that is not there. */
  'sds-progress',
  /* And the stops that job consists of, for the same reason from the other
     end. A run arrives one stop at a time and ends on a verdict, and nothing
     arrives in a document. The instruction a manual does have is `sds-steps`. */
  'sds-run',
...
/* The shell every page layout under `specimens/screens/` consists of. The
   ...
   The bar is the one of them a template addresses rather than writes. What a
   header does as it runs out of room comes from a measurement. A template
   that spells `.sds-bar` by hand is a row that cannot fold. */
...
/* Twig comments are prose about the template. They name things the template
   does not write: the core class it replaces, the element it deliberately is
   not. Read as markup, every explanation is a violation. */
...
const shown = [
  ...
  /* An element's own source: the classes it emits draw wherever it is. */
...
/* Only the fixture's own documents. `acceptance/_cards/` is a copy of the
   specimen cards, there so a guideline page can embed one. The theme never
   rendered a component that appears only in a copied card. */
...
function pending(list: readonly string[], covered: (name: string) => boolean, axis: string): Set<string> {
  for (const name of list) {
    if (covered(name)) fails.push(`${name} has its place now — take it out of PENDING.${axis} in scripts/coverage.ts`);
  }
  return new Set(list);
}

/* Six questions, each its own row and its own numbers. `fails` is the whole
   run's, so an axis owns whatever arrived while it ran. That is what lets the
   row that failed be the row the findings sit under. */
const AXES = [
  { name: 'stories', label: 'a story for every element' },
  { name: 'classes', label: 'something draws every class the system defines' },
...
report.align(AXES);
report.open('coverage', 'every component has its three places');
...
for (const tag of TAGS) {
  if (!hasStory(tag) && !missing.has(tag)) fails.push(`${tag}: no story names it — every element has a story`);
}
...
/* The specimen chrome and the document layer are not the class vocabulary a
   design draws with. So the set that needs a *drawing* leaves them out. */
...
for (const cls of classes) {
  if (!isDrawn(cls) && !missing.has(cls)) fails.push(`.${cls} has a definition but no story, card, page or element draws it`);
}
axis('classes', `${classes.filter(isDrawn).length} of ${classes.length} classes`);

/* What each element renders, read out of its own source. A template that names
   an element puts everything that element draws on the page too. That is how
   most of them arrive now that the theme addresses components. The walk along
   the composition keeps this a question of "is it in the render" rather than
   "is its name in a template". */
...
/* What a helper under `src/lib/` writes on the element's behalf, read as the
   ...
   copy in each. Without this, a part moved out takes whatever it draws out of
   the render while the page it produces stays exactly the same. */
...
for (const file of walk(join(FRONTEND, 'src', 'components'), ['.ts'])) {
  ...
  /* Comments first, and they are the whole reason this is not a plain match.
     ...
     Read as composition, those credit half the system to any file that
     explains itself. What remains is markup. */
...
/* One walk out from what the theme names. So an element three deep in a
   composition gets the same credit as one the template wrote itself. */
  ...
  if (inGuides(tag)) {
    fails.push(
      `${tag} is in the render — it is not outside a document after all. ` +
        'Say where it belongs in scripts/coverage.ts and take it out of ELSEWHERE.',
    );
  }
}
for (const tag of TAGS) {
  if (!inGuides(tag) && !missing.has(tag) && !elsewhere.has(tag)) {
    fails.push(`${tag}: neither a template emits it nor the fixture asks for it — no document tests it`);
  }
}
axis(
  'render',
...
/* A part belongs to the element that draws it and to nothing else. A template
   ...
   layer's vocabulary, which the theme can write. */
for (const m of theme.matchAll(/\b(sds-[a-z-]+)__[a-z-]+/g)) {
  const owner = m[1] as string;
  /* `TAGS` is a tuple of literals, so the tag needs a wider type before the
     question. The question is if a string is one of them, which is exactly
     what the narrow type will not let a caller ask. */
...
for (const m of theme.matchAll(/class="([^"]*)"/g)) {
  /* Half of these attributes take shape at render time. The literal parts of
     one are still names and get the check. What an expression evaluates to is
     unknown here, so the expression goes and what remains has to look like a
     class name. An attribute this cannot read is not a licence to invent one;
     it is out of reach of this check. */
...
/* `language-*` is the fence's grammar rather than a hook. The name every
   ...
   reason; here the language is a binding, so only the stem remains to see. */
...
for (const cls of SHELL) {
  if (!theme.includes(cls)) {
    fails.push(`the theme names no .${cls} — a page consists of the shell the screens share, not one of its own`);
  }
}
