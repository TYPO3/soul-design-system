...
/* Compare two screenshot dirs and report what moved.
   ...
   One line per card: identical, or the share of pixels that changed. Under a
   changed one the three files to open — before, after, and the mask that
   marks what moved. A refactor meant to be neutral prints all-identical. */
...
/* Every path prints from the root, because a reader opens it from there. */
...
for (const n of names) {
  ...
  /* Exact comparison keeps small token changes visible; a tolerance can erase
     ...
     than hide it behind a threshold. */
