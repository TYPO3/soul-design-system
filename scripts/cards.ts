...
if (import.meta.url === pathToFileURL(process.argv[1] ?? '').href) {
  ...
  report.open('cards', check ? 'every card matches its story, and has one' : 'generate the cards from their stories');
  report.align(results.map((r) => ({ name: r.changed ? (r.existed ? 'updated' : 'created') : 'unchanged', label: r.path })));

  /* Only a write says what it did to each file. `--check` answers one question
     about the tree, and sixty lines saying "unchanged" is not the answer. */
  if (!check) {
    for (const r of results) {
      report.row(r.changed ? 'ok' : 'skip', r.changed ? (r.existed ? 'updated' : 'created') : 'unchanged', r.path, `← stories/${r.file}`);
    }
  }
  ...
  const problems = [
    ...(check ? stale.map((r) => `${r.path} no longer matches stories/${r.file}`) : []),
    ...stray.map((rel) => `${rel} is generated by no story — write the one that produces it, or delete the file`),
  ];
  report.summary(
    `${results.length} cards · ${stale.length} ${check ? 'stale' : 'written'} · ${stray.length} orphaned`,
    problems,
  );
