/* What a whole page is, twice.
   ...
   field takes text. A layout nobody can click through is where a broken
   control hides, and the test suite opens every story. In `screens/` it has
   to be **static**: the pane opens those files with no JavaScript, and
...
export interface PageMode {
  /** True while the file under `screens/` composes. */
  flat?: boolean;
}

/* And what every page has whatever site it belongs to. The way past the bar,
   the rail and the breadcrumbs, which stand between the top of the page and
   the text of it. One target name here and in the guides theme, so a reader
   who has learned the press has learned it everywhere. */
