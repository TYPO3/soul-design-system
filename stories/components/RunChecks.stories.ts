/* The other shape a run takes: many jobs at once, grouped by what has become
   ...
   changes is that the order says nothing. No job waits for another, so their
   state sorts them. The group carries the count, so one folded away still
   says what is inside it.

   And each row says in words what a mark cannot. "Queued" and "started now"
   tell a check nobody has picked up from one in hand. That is one glyph
   apart and a whole answer apart. */
...
const CHECKS: readonly RunStep[] = [
  { group: '2 queued', label: 'Build PHP (8.4)', state: 'ahead', note: 'In the queue for this check' },
  { group: '2 queued', label: 'Build PHP (8.5)', state: 'ahead', note: 'In the queue for this check' },
...
/** Mid-flight. Every group stands open while the work goes on. A reader who
    watches a run wants the rows, and folds them once they have seen which
    one matters. */
