/* A page found in a site that has no server.
   ...
   behind these stories, which is why a keystroke here gets the sentence a
   search with no hits gives. That is the one state a specimen can show with
   no build behind it.

   Its home is the bar, and `sds-nav-main` puts it there and takes it away again
   as the row runs out of room. No `parameters.dsCard`: what a card holds is
   an empty text field. */
...
/** The field on its own. It draws the drop under itself rather than under
    whatever box happens to sit above it. The arrows walk the hits — a reader
    who has arrowed to one can open it like any other link. */
...
/** The box is `sds-field`, so it takes that element's three heights. `sm` is
    for a search inside another surface — a bar whose other controls are
    small. The drop keeps the field's width whatever the height is. */
...
/** Where it stands in practice. The bar measures the row and the field is the
    first thing it can do without. Below that width it is in the drawer,
    whole, rather than squeezed to a box too narrow to read the text in it. */
