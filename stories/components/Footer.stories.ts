...
/** Everything this component places, in the order it places it. The lockup and
    ...
    end, where a reader looks for them by position. The columns reflow
...
/** The same accounts as a column instead, which is the other place they can
    go. There they are links with labels: a reader reads a column, where they
    look for the row at the end of the line. Each glyph leads its label and
    `actions-window-open` follows it, to say the link leaves the site. */
...
/** Without the trailing links. The note stays: it is the one part of this
    component a page must not leave out. The closing line goes with them
    rather than leaves a rule across the page under nothing. */
...
/** The closing line as the whole footer: whose it is, what has to travel with
    it, and where else it lives. Nothing stands set above it, so there is no
    block above it. Right for a surface that has already said what it is
    further up the page, and for a site whose sections are in the bar. */
...
/** The marks with nothing beside them on the line. They sit where the line
    ...
    *of*. A set of marks against the right edge of a footer that says nothing
    else reads as something that lost what it belonged to. */
export const Marks: Story = {
  args: { groups: [], meta: [], copyright: '' },
};

/** A screen with no site around it: what it is, the sentence it owes, and the
    way out. Nothing else has a value, so the columns, the mark and the notice
    are not there. The same element, and the ending a landing page gets. */
