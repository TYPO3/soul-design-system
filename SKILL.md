...
**A drawing of your own is referenced too.** Name its root `<svg id="soul-ref" viewBox="…">` and write every colour as a `var()` with a hex behind it. The viewBox on that root is what the shapes are drawn in, and the build puts the same box on the wrapper, because a reference carries no coordinate system across and a wrapper without one has no shape to hold. Never a `<style>` block and never a colour on the root: both are applied to the shapes after they are referenced, both beat a presentation attribute, and the drawing is then the author's grey on every page there is. A comment may not contain a double dash either — that is malformed XML, and the file draws nothing anywhere. `docs/design-system/artwork.rst` is the whole rule; a signet a documentation project configures follows it.
