...
**A titled block carries a pair, not two sizes.** Anything with a heading over its own text takes one of two registers, and both halves move together. A *block* — note, surface, empty state, modal, accordion — is `--block-title-size` over `--block-body-size`. An *entry*, whose title is a destination you open — card, teaser, result — is `--entry-title-size` over that same body. Both rebind at once wherever a block is being *read* — a `.sds-prose` document or a `.sds-column` reading column — so it takes the page's own size there. What does not rebind is machine content: a code block, a compact row and a caption keep the dense register in the same paragraph. Never set a block's title from `--font-size-*` directly: that is exactly how a title ends up a register away from the text beneath it.
