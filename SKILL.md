...
**Where they come from.** The set is generated and committed: `scripts/icons.ts` copies the identifiers it declares out of the `@typo3/icons` npm package (`^5.0.3`, MIT), which the container installs and its entrypoint materialises, and `make icons` is what writes them into the tree. Committed because the package publishes them and nothing on the way out builds — a mirror replays what git has. A file missing under `packages/frontend/assets/icons/` is `make icons` not having been run after a change to `CATEGORIES`, not a gap to work around by hand.

**How to get one outside `actions`.** `packages/frontend/assets/icons/icons.json` lists what is here, and its paths resolve against it. For an icon that is not here yet, read the upstream manifest instead — `node_modules/@typo3/icons/dist/icons.json` in the container, or <https://typo3.github.io/TYPO3.Icons/> — which names an identifier's category, so nothing has to be guessed from the spelling. Nothing is copied out of it into this tree: a second list is a list that goes stale, and its paths are the upstream package's rather than this one's. The categories are `actions`, `apps`, `avatar`, `content`, `default`, `files`, `form`, `information`, `install`, `mimetypes`, `miscellaneous`, `module`, `overlay`, `spinner` and `status`; that file maps every identifier to its category and every deprecated alias to its current name. Resolve an alias before using it; the old spelling is not what `typo3_icon_lookup` returns.
