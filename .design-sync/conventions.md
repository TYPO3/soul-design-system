...
| Values a reader takes away | `<sds-copy label="Directory" value="~/site">` — the value in the machine's own font with the button that puts it on the clipboard, on one line. `sds-code` is the block form and is a frame around a frame when it is one word. `sds-copy__value|__button` — the press is one glyph and its sentence is `title` |
