...
| Decks | `sds-slide`. One 16:9 frame: the page at twice the size, so every element between the tags keeps its set. `kind` is `cover`, `section`, `statement`, `content` or `closing`. `ground` is `paper` unless said, and `terminal` on the cover. `eyebrow`, `heading`, `lead`, `note`, `number`. The lockup as `signet`, `brand`, `product`; the outline as `sections` and `current`. `fit` scales the frame to the room it has. |
