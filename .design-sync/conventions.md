...
| Fields | `sds-field` + `--sm` `--lg` `--multi` — the control heights a button has, `md` unless one is said, so a field and the button beside it stand on one line; `sds-input` `sds-field__affix` `sds-select` `sds-select__mark` `sds-field-error` `sds-field-group`; `sds-switch` + `sds-switch__track` `sds-switch__body` `sds-switch__label` `sds-switch__hint`; `sds-range` + `sds-range__head` `sds-range__value` `sds-range__slider`; `sds-file` |
