...
| Long text | `sds-quote` `sds-byline` `sds-confval`. The first two take `as` for what the source is. `role` is the ARIA attribute and is out of reach. `sds-confval` is one configuration value in a reference. `name`, `anchor`, `required`, `type`, `default`, and `facts` for whatever else the source named |
