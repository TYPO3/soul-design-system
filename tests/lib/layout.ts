/* What a page measures for, wherever it came from.
   ...
   measurement rather than a look. From the story layouts in `pages.test.ts`,
   inside the frame. And through Playwright from the rendered documentation
   in `guides.spec.ts`, which is the same page in somebody else's hand.

   Each measurement is one function of the page, whole: no import and no
   value from outside it. Playwright sends the function's own text into the
   page, and a name it does not carry is a name the page does not have. */

import type { Page } from '@playwright/test';

export function pageOverflow(): { scroll: number; client: number; worst: string } | null {
...
export function pageOverlaps(): string[] {
...
export function pageClipped(): string[] {
...
/* The same three, asked of a page Playwright holds. */
export const overflowOf = (page: Page): Promise<ReturnType<typeof pageOverflow>> => page.evaluate(pageOverflow);
export const overlapsOf = (page: Page): Promise<string[]> => page.evaluate(pageOverlaps);
export const clippedOf = (page: Page): Promise<string[]> => page.evaluate(pageClipped);
